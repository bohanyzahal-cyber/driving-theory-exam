# Reliability release — 2026-09-05-r6
...
- r6: rollback of two mitigations that were added while the cache was unreadable. (1) The practice block during open exam sessions is OFF by default; set the Script Property `PRACTICE_BLOCK_DURING_EXAMS` to `on` to re-enable it without a deployment (effective within about 60 s). (2) Approval polling (examinee) and dashboard polling (examiner) return to a 5 s base interval; the adaptive slowdown to 20 s after a slow or failed response is unchanged. Service-worker cache names were bumped (`examinee-v8`, `examiner-v42`) so the fleet picks up the client change.
- r5: **every CacheService read of a gzip record had failed in production since r1.** `decodeQuestionCache` built the Blob without a content type and `Utilities.ungzip` rejects that ("Blob object must have non-null content type for this operation"). Reads of pools, banks and translation shards all threw, were logged as `invalid/read failed`, and were treated as cache misses: each request rebuilt its pool from Drive under a lease (4-6 s), concurrent requests received `question_cache_busy`, and exam starts fell back to the seven-bank path. The write-time verification compared strings only, so warmup reported `ready: true` while nothing was readable. Fixed by giving the Blob the `application/x-gzip` type; write verification and `questionCacheStatus` now decode a real record (`decodeChecks`/`decodeFailures`). The Node harness models the content-type requirement so this cannot pass offline again. Diagnosed from the editor with a function that ran the reader step by step and logged the exception.
...
3. Check that the public health response reports `build: "2026-09-05-r6"`. A successful Pages deployment does not prove this server step occurred.
