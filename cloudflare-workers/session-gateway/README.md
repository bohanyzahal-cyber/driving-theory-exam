...
> **r32.1 (22/09 16:00):** ת.ז. שאין לה שורה בעותק שה-Worker מחזיק = **דחיפת ההרשמה** (הטלפון מודיע ברגע שהשרת אישר, לפני שה-Worker קרא את השורה). עד r32.1 זה נדחה ב-403 והלוח חיכה לקריאת הביטחון (נמדד 15:25); עכשיו זה drop בתקציב `REREAD_GAP_MS` — אותו תקציב כמו דחיפה ריקה של בוחן, ולכן ת.ז. מומצאת לא יכולה לקנות יותר מהרצה אחת ב-2 שנ׳ לסשן. טוקן שגוי לשורה **קיימת** נשאר 403.
