...
1. **מאחד את סקרי הנבחנים:** קריאה אחת ל‑Apps Script לכל סשן, כמה נבחנים שלא יסקרו —
   ...
   עצמה מקבלת עותק עד 45 שניות, וכל כתיבה דוחפת. במקום ~480 הרצות לדקה ל‑40 נבחנים — **הרצה
   אחת ל‑45 שניות** לכיתה שקטה.
...
| (ד) **קצב ביטחון** `HELD_REREAD_MS` = **45 שניות** | בקשה **עם** `fp` (חימוש מחדש של שרשרת, וכל סבב של החזקה) מקבלת עותק עד 45 שניות — הלקוח כבר מחזיק בדיוק את המצב הזה, ורק שינוי מעניין אותו |
...
**עשר דקות בלי שום אירוע** — **14 הרצות Apps Script** במקום ~300. כיתה שלמה שלא קורה בה כלום
עולה לגוגל פעם ב‑45 שניות, ולא 30 פעמים בדקה.

**מה זה עולה:** שינוי ש**אף אחד לא דחף** (תיקון ידני בגיליון, השלמה אוטומטית של שורה תקועה)
נראה תוך עד 45 שניות במקום עד 2. כל שינוי שמגיע מהמערכת עצמה — החלטת בוחן, הרשמה, הגשה,
פסילה — מגיע **מיד**, כי מי שכתב אותו גם דחף. לוח הבוחן ממילא מחזיק רשת ביטחון משלו (60
שניות, §13.2).

**רמז למי שקורא לוג:** `age` בשורת היומן הוא גיל העותק שענה. `age` שמטפס עד ~45,000 ואז
...
| `age` | גיל העותק שענה (ms). מטפס ל‑45,000 ומתאפס = הקצב הפסיבי; קטן = מישהו דחף |
...
מאז r31.2 (קריאה רק כשקרה משהו) כיתה שלמה בלי אירועים עולה **הרצת Apps Script אחת ל‑45
שניות** במקום 30 לדקה — פי ~20 פחות מול גוגל; מספר הבקשות ל‑Cloudflare לא השתנה.
