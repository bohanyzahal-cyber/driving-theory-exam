...
| `POST /v1/invalidate?sessionCode=ABC12345` | — | `{"status":"ok"}` |
...
ה‑Worker מוחק את ה‑snapshot של הסשן מהזיכרון ומשתי רשומות ה‑Cache (`snap`, `stale`), כך
שהסקר הבא של הנבחן קורא מהשרת מיד במקום לחכות לתום חלון ה‑2 שניות. **מוגבל לקריאה חוזרת
כפויה אחת לכל 2 שניות לסשן** — אותו תקציב שממנו שואב גם הרענון האוטומטי של "שורה שחסרה
ב‑snapshot", כך שסופת `invalidate` לא יכולה להכפיל את העומס על Apps Script. התשובה תמיד
`{"status":"ok"}`, גם כשהמגבלה בלמה את המחיקה.
